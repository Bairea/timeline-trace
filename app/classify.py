...
**已知局限（未解决，见 docs/reviews/2026-09-20-template-config-review.md §4）**：

  长模板块里的短记录、且名字与模板名无关时，第 3 条会落榜。
  例如「工作」模板块 210 分钟，用户记「专注写代码 30 分钟」→ 归入「其他」，
  接不进项目推进统计。而「我今天工作了半小时」是完全正常的记录方式。

  这需要一套分层判定（完整落在模板块内时按占实际块比例，跨越边界时
  按占模板块比例）才能兼顾，但会重新引入「短视频被吸收」的问题，
  需要额外判据。**这是一个待决策的语义取舍，不是可以随手改的参数。**
