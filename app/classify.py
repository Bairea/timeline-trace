...
**规则 1、2 刻意不看时间**，这是个容易被「顺手修正」掉的决定，故记下理由：

  名字不是「匹配模板块」的线索，而是用户对**这是什么事**的直接声明。
  用户打了「干活」，就是在说他处于干活状态——时间放在上午还是晚上，
  不影响这件事的分类。若要求名字匹配必须与模板块时间重叠，就会丢掉：
  「785–845 干活」（模板里上午叫「工作」、干活排在 20:30）——
  而**时间错位恰恰是这个工具最该暴露的东西**（对照引擎的存在意义），
  统计层不该把它悄悄丢掉。

  时间只在没有名字线索时才作为推断依据（规则 3）——因为那时
  除了「这件事发生在哪个模板块时段内」，确实没有别的信息可用。

**已裁定的取舍（不要"顺手修正"，见 docs/reviews/...-review.md §4）**：

  长模板块里的短记录、且名字与模板名无关时，第 3 条会落榜。
  例如「工作」模板块 210 分钟，用户记「专注写代码 30 分钟」→ 归入「其他」，
  接不进项目推进统计。

  曾考虑分层判定（完整落在模板块内时按占实际块比例，跨越边界时按占
  模板块比例）来兼顾，但会重新引入「短视频被吸收」的问题，需要额外判据；
  且「模板块内时长最长者继承」这类相对判据不可预测（块内只有一条记录时
  无论多离谱都会归位）。**2026-09-20 用户裁定：接受现状，不修。**

  理由：这条统计的角色是**核对计划是否被履行**，不是给当天行为做归类。
  一条不含名字线索的短记录，本就无法断言它履行了哪项计划。
  要让它进统计，**记录时带上可匹配的名字即可**——但注意规则 2 只认
  模板名的**首段主干**：「干活」「干活写代码」能命中「干活（中间做拉伸）」，
  而「写代码」「拉伸」「中间做拉伸」都不能（括注被 _base_name 剥掉）
  ——这是记录习惯问题，不是算法问题。

  行为已由 tests/test_stats.py::test_unknown_name_short_record_falls_through
  固定，改动若破坏它会红。**若「其他」占比异常高，先查记录时是否没写名字。**

为什么规则 3 要卡「占模板块比例」而不是「占实际块比例」：
